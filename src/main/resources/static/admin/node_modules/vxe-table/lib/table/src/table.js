...
function renderEmptyContenet(h, _vm) {
  ...
  if ($scopedSlots.empty) {
    ...
    var renderTableEmptyView = compConf ? compConf.renderTableEmptyView || compConf.renderEmpty : null;
    if (renderTableEmptyView) {
      emptyContent = (0, _vn.getSlotVNs)(renderTableEmptyView.call(_vm, h, emptyOpts, params));
    } else {
...
var _default2 = exports.default = {
  ...
  computed: {
    ...
    tipConfig: function tipConfig() {
      return _objectSpread({}, this.tooltipOpts);
    },
    ...
    autoWidthColumnList: function autoWidthColumnList() {
      var visibleColumn = this.visibleColumn;
      return visibleColumn.filter(function (column) {
        return column.width === 'auto';
      });
    },
    fixedColumnSize: function fixedColumnSize() {
    ...
    isMaxFixedColumn: function isMaxFixedColumn() {
      var maxFixedSize = this.columnOpts.maxFixedSize;
      if (maxFixedSize) {
  ...
  watch: {
    ...
    reColumnFlag: function reColumnFlag() {
      var _this3 = this;
      this.$nextTick().then(function () {
        return _this3.refreshColumn();
      });
    },
    ...
    showFooter: function showFooter() {
      var _this5 = this;
      this.$nextTick(function () {
        _this5.recalculate(true).then(function () {
          return _this5.refreshScroll();
        });
      });
    },
    ...
    syncResize: function syncResize(value) {
      ...
      if (value) {
        ...
        this.$nextTick(function () {
          handleUupdateResize(_this8);
          setTimeout(function () {
            return handleUupdateResize(_this8);
          });
        });
      }
    },
    mergeCells: function mergeCells(value) {
      ...
      this.$nextTick(function () {
        return _this9.setMergeCells(value);
      });
    },
    mergeFooterItems: function mergeFooterItems(value) {
      ...
      this.$nextTick(function () {
        return _this10.setMergeFooterItems(value);
      });
    }
  },
  created: function created() {
    ...
    if (process.env.NODE_ENV === 'development') {
      ...
      if (importConfig && importOpts.types && !importOpts.importMethod && !_xeUtils.default.includeArrays(_xeUtils.default.keys(importOpts._typeMaps), importOpts.types)) {
        (0, _log.warnLog)('vxe.error.errProp', ["export-config.types=".concat(importOpts.types.join(',')), importOpts.types.filter(function (type) {
          return _xeUtils.default.includes(_xeUtils.default.keys(importOpts._typeMaps), type);
        }).join(',') || _xeUtils.default.keys(importOpts._typeMaps).join(',')]);
      }
      if (exportConfig && exportOpts.types && !exportOpts.exportMethod && !_xeUtils.default.includeArrays(_xeUtils.default.keys(importOpts._typeMaps), exportOpts.types)) {
    ...
    this.loadTableData(data).then(function () {
      if (data && data.length) {
        _this11.inited = true;
        _this11.initStatus = true;
        _this11.handleLoadDefaults();
        _this11.handleInitDefaults();
      }
      _this11.updateStyle();
    });
  ...
  mounted: function mounted() {
    ...
    if (this.autoResize) {
      var handleWrapperResize = this.resizeOpts.refreshDelay ? _xeUtils.default.throttle(function () {
        return _this12.recalculate(true);
      }, this.resizeOpts.refreshDelay, {
        leading: true,
        trailing: true
      }) : null;
      var resizeObserver = (0, _resize.createResizeEvent)(handleWrapperResize ? function () {
        if (_this12.autoResize) {
  ...
  render: function render(h) {
    ...
    var _e = this._e,
      ...
      tId = this.tId,
      isCalcColumn = this.isCalcColumn,
      tableData = this.tableData,
    ...
    var currLoading = this._isLoading || loading;
    return h('div', {
      class: ['vxe-table', 'vxe-table--render-default', "tid_".concat(tId), vSize ? "size--".concat(vSize) : '', "border--".concat(tableBorder), (_ref = {}, _defineProperty(_defineProperty(_defineProperty(_defineProperty(_defineProperty(_defineProperty(_defineProperty(_defineProperty(_defineProperty(_defineProperty(_ref, "valid-msg--".concat(validOpts.msgMode), !!editRules), 'vxe-editable', !!editConfig), 'old-cell-valid', editRules && _conf.default.cellVaildMode === 'obsolete'), 'cell--highlight', highlightCell), 'cell--selected', mouseConfig && mouseOpts.selected), 'cell--area', mouseConfig && mouseOpts.area), 'row--highlight', rowOpts.isHover || highlightHoverRow), 'column--highlight', columnOpts.isHover || highlightHoverColumn), 'checkbox--range', checkboxOpts.range), 'column--calc', isCalcColumn), _defineProperty(_defineProperty(_defineProperty(_defineProperty(_defineProperty(_defineProperty(_defineProperty(_defineProperty(_defineProperty(_defineProperty(_ref, 'is--header', showHeader), 'is--footer', showFooter), 'is--group', isGroup), 'is--tree-line', treeConfig && (treeOpts.showLine || treeOpts.line)), 'is--fixed-left', leftList.length), 'is--fixed-right', rightList.length), 'is--animat', !!this.animat), 'is--round', this.round), 'is--stripe', !treeConfig && stripe), 'is--loading', currLoading), _defineProperty(_defineProperty(_defineProperty(_defineProperty(_defineProperty(_ref, 'is--empty', !currLoading && !tableData.length), 'is--scroll-y', overflowY), 'is--scroll-x', overflowX), 'is--virtual-x', scrollXLoad), 'is--virtual-y', scrollYLoad))],
      on: {
