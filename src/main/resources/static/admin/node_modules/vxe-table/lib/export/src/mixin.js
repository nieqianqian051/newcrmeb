...
function toCsv($xetable, opts, columns, datas) {
  ...
  if (opts.isHeader) {
    content += columns.map(function (column) {
      return toTxtCellLabel(getHeaderTitle($xetable, opts, column));
    }).join(',') + enterSymbol;
  }
...
function createHtmlPage(opts, content) {
  ...
  return ['<!DOCTYPE html><html>', '<head>', '<meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,minimum-scale=1,maximum-scale=1,user-scalable=no,minimal-ui">', "<title>".concat(opts.sheetName, "</title>"), '<style media="print">.vxe-page-break-before{page-break-before:always;}.vxe-page-break-after{page-break-after:always;}</style>', '<style>.vxe-table{white-space:pre;}</style>', "<style>".concat(defaultHtmlStyle, "</style>"), style ? "<style>".concat(style, "</style>") : '', '</head>', "<body>".concat(content, "</body>"), '</html>'].join('');
}
    ...
    if (isColgroup && !original) {
      colgroups.forEach(function (cols) {
        tables.push("<tr>".concat(cols.map(function (column) {
          ...
          var cellTitle = getHeaderTitle($xetable, opts, column);
          var childWidth = 0;
          var countChild = 0;
...
function toXML($xetable, opts, columns, datas) {
  ...
  if (opts.isHeader) {
    xml += "<Row>".concat(columns.map(function (column) {
      return "<Cell><Data ss:Type=\"String\">".concat(getHeaderTitle($xetable, opts, column), "</Data></Cell>");
    }).join(''), "</Row>");
  }
  datas.forEach(function (row) {
...
function handleExportAndPrint($xetable, options, isPrint) {
  ...
  var proxyOpts = $xegrid ? $xegrid.proxyOpts : {};
  ...
  var typeList = types.map(function (value) {
    return {
      value: value,
      label: _conf.default.i18n("vxe.export.types.".concat(value))
    };
  });
  var modeList = modes.map(function (item) {
    if (item && item.value) {
      return {
        value: item.value,
    ...
    return {
      value: item,
      label: _conf.default.i18n("vxe.export.modes.".concat(item))
    };
  });
  // 默认选中
  ...
  if (!modeList.some(function (item) {
    return item.value === exportParams.mode;
  })) {
    exportParams.mode = modeList[0].value;
  }
  if (!typeList.some(function (item) {
    return item.value === exportParams.type;
  })) {
  ...
  methods: {
    ...
    _openImport: function _openImport(options) {
      ...
      var modeList = modes.map(function (item) {
        ...
        return {
          value: item,
          label: _conf.default.i18n("vxe.import.modes.".concat(item))
        };
      });
      ...
      if (!modeList.some(function (item) {
        return item.value === _this4.importParams.mode;
      })) {
