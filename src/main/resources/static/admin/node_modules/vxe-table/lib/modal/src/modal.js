...
var allActivedModals = exports.allActivedModals = [];
var msgQueue = exports.msgQueue = [];
var _default2 = exports.default = {
  ...
  props: {
    ...
    showConfirmButton: {
      type: Boolean,
      default: function _default() {
        return _conf.default.modal.showConfirmButton;
      }
    },
    confirmButtonText: {
      type: String,
  ...
  render: function render(h) {
    ...
    return h('div', {
      class: ['vxe-modal--wrapper', "type--".concat(type), className || '', (_ref = {}, _defineProperty(_defineProperty(_defineProperty(_defineProperty(_defineProperty(_defineProperty(_defineProperty(_defineProperty(_defineProperty(_defineProperty(_ref, "size--".concat(vSize), vSize), "status--".concat(status), status), 'is--animat', animat), 'lock--scroll', lockScroll), 'lock--view', lockView), 'is--resize', resize), 'is--mask', mask), 'is--maximize', revertLocat), 'is--visible', contentVisible), 'is--active', visible), _defineProperty(_ref, 'is--loading', loading))],
      style: {
      ...
      on: headerOns
    }, headerSlot ? !initialized || destroyOnClose && !visible ? [] : (0, _vn.getSlotVNs)(headerSlot.call(this, {
      $modal: this
    }, h)) : [h('div', {
      ...
      class: ['vxe-modal--zoom-btn', 'trigger--btn', revertLocat ? _conf.default.icon.MODAL_ZOOM_OUT : _conf.default.icon.MODAL_ZOOM_IN],
      attrs: {
        title: _conf.default.i18n("vxe.modal.zoom".concat(revertLocat ? 'Out' : 'In'))
      },
      on: {
    ...
    }, defaultSlot ? !initialized || destroyOnClose && !visible ? [] : (0, _vn.getSlotVNs)(defaultSlot.call(this, {
      $modal: this
    ...
    }, footerSlot ? !initialized || destroyOnClose && !visible ? [] : (0, _vn.getSlotVNs)(footerSlot.call(this, {
      $modal: this
    }, h)) : [_xeUtils.default.isBoolean(showCancelButton) ? showCancelButton : type === 'confirm' ? h('vxe-button', {
      key: 1,
      ref: 'cancelBtn',
      on: {
        click: this.cancelEvent
      }
    }, this.cancelButtonText || _conf.default.i18n('vxe.button.cancel')) : null, _xeUtils.default.isBoolean(showConfirmButton) ? showConfirmButton : type === 'confirm' || type === 'alert' ? h('vxe-button', {
      key: 2,
      ref: 'confirmBtn',
      props: {
        status: 'primary'
    ...
    }, this.confirmButtonText || _conf.default.i18n('vxe.button.confirm')) : null]) : null, !isMsg && resize ? h('span', {
  ...
  methods: {
    ...
    revert: function revert() {
      var _this8 = this;
      return this.$nextTick().then(function () {
        var revertLocat = _this8.revertLocat;
        if (revertLocat) {
          var modalBoxElem = _this8.getBox();
          _this8.revertLocat = null;
          Object.assign(modalBoxElem.style, {
    ...
    zoom: function zoom() {
      var _this9 = this;
      return this[this.revertLocat ? 'revert' : 'maximize']().then(function () {
    ...
    mousedownEvent: function mousedownEvent(evnt) {
      ...
      var remember = this.remember,
        ...
        revertLocat = this.revertLocat;
      var modalBoxElem = this.getBox();
      if (!revertLocat && evnt.button === 0 && !_dom.default.getEventTargetNode(evnt, modalBoxElem, 'trigger--btn').flag) {
        evnt.preventDefault();
    ...
    dragEvent: function dragEvent(evnt) {
      ...
      document.onmousemove = function (evnt) {
        ...
        if ($listeners.resize) {
          _this12.$emit('resize', params);
        } else if (events.resize) {
          events.resize.call(_this12, params);
        }
      };
      document.onmouseup = function () {
        _this12.revertLocat = null;
        document.onmousemove = domMousemove;
        document.onmouseup = domMouseup;
        setTimeout(function () {
          modalBoxElem.className = modalBoxElem.className.replace(/\s?is--drag/, '');
    ...
    savePosStorage: function savePosStorage() {
      ...
      if (remember && storage) {
        ...
        posStorageMap[id] = [modalBoxElem.style.left, modalBoxElem.style.top, modalBoxElem.style.width, modalBoxElem.style.height].concat(revertLocat ? [revertLocat.left, revertLocat.top, revertLocat.width, revertLocat.height] : []).map(function (val) {
