<template>
    <div class="news-box" :style="{
		...
		<div class="item" :style="{
			...
			<div class="top" v-else :style="{
				color:toneConfig?titleColor:'#fff',
				background: toneConfig?`linear-gradient(90deg,${titleBgColorLeft} 0%,${titleBgColorRight} 100%)`:colorStyle.theme
			}">{{titleTxtConfig || '商城头条'}}</div>
		    <div class="right-box" :style="{
				color:newsColor
			}">{{list[0].chiild[0].val}}</div>
			<span class="iconfont iconjinru" :style="{
				color:bntColor
			}" v-if="!buttonConfig"></span>
		</div>
		<div class="list" v-else :style="{
			background: `linear-gradient(90deg,${bgColorLeft} 0%,${bgColorRight} 100%)`,
			borderRadius:bgRadius
		}">
			<div class="title acea-row row-between-wrapper">
				...
				}">{{textConfig}}<span class="iconfont iconjinru"></span></div>
			</div>
			<div class="text" v-for="(item, index) in list" :key="index" :style="{
				color:toneConfig?newsColor:colorStyle.theme
			}"><span class="num" :class="index == 0?'on':index == 1?'on2':index == 2?'on3':''">{{index+1}}</span>{{item.chiild[0].val}}</div>
		</div>
    </div>
</template>
