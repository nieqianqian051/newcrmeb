      ...
      >
        <Row :gutter="24" type="flex" v-show="currentTab === '1'">
          ...
          <Col span="24" v-if="formValidate.video_open">
            <FormItem label="上传视频：" prop="video_link">
              ...
              <div class="tips">
                建议使用云存储保存视频，<router-link
                  :to="{ path: '/admin/setting/storage' }"
                  >配置云存储</router-link
                >，建议时长：9～30秒，视频宽高比16:9
                <Poptip
                  placement="bottom"
                  trigger="hover"
                  width="440"
        ...
        <Row :gutter="24" type="flex" v-show="currentTab === '2'">
          ...
          <Col
            span="24"
            v-if="
              formValidate.spec_type === 1 && formValidate.product_type != 4
            "
            class="noForm"
          >
            <Col span="24">
                      ...
                      <div class="rulesBox">
                        <draggable :list="item.detail" handle=".drag">
                          <Tag
                            ...
                            @dblclick.native="handleDblclick(j, indexn, index)"
                          >
                            <Input
                              ref="tagInput"
                              v-if="indexn === editTagIndex[1] && index === editTagIndex[0]" 
                              v-model="editTagText"
                              style="width: 100px;"
                              @on-enter="handleEnter(editTagText, j)"
                              @on-blur="handleEnter(editTagText, j)"
                            />
                            <span v-else>{{ j }}</span>
                          </Tag>
                        </draggable>
              ...
              <Col :xl="6" :lg="9" :md="10" :sm="24" :xs="24">
                <FormItem label="规格值：">
                  <Input
                    ...
                    placeholder="请输入规格值"
                    @on-enter="handleAttrEnter(formDynamic.attrsVal)"
                  />
                </FormItem>
              </Col>
                ...
                <FormItem label="批量设置：" class="labeltop">
                  ...
                  >
                    ...
                    <template slot-scope="{ row, index }" slot="pic">
                      ...
                      >
                        <div
                          class="pictrue pictrueTab have-btn"
          ...
          <div
            v-if="
              formValidate.spec_type === 0 || formValidate.product_type == 4
            "
            style="width: 100%;"
          >
            <Col span="24" v-if="formValidate.product_type != 4">
          ...
          <Col span="24" v-if="formValidate.is_sub.length">
            ...
            <FormItem label="批量设置：" v-if="formValidate.spec_type === 1">
              <span v-if="formValidate.is_sub.indexOf(1) > -1">
                一级返佣：
                <div class="input-number columnsBox perW20">
                  <InputNumber
                    placeholder="请输入一级返佣"
                    :min="0"
        ...
        <Row v-show="currentTab === '6'">
              ...
              <div class="tips">
                ...
                >
                  ...
                  <div class="exampleImg" slot="content">
                    <img
                      :src="
                        `${baseURL}/statics/system/productRecommendImage.png`
                      "
                      alt=""
                    />
                  </div>
          ...
          <Col span="24">
            <FormItem label="自定义留言：">
              ...
              <router-link
                class="addClass"
                :to="{ path: '/admin/setting/system_form' }"
                >新增表单</router-link
              >
  ...
  methods: {
        ...
        if(newData.length){
          newData.forEach(item=>{
            if(j.values.split('').length == item.split('').length){
              ...
              if(j.values.split(',').indexOf(item) != -1){
                j.select = true;
              }
            }
          })
    ...
    addmanyData(data, type) {
      if (type == 1) {
        ...
        data.forEach((item) => {
          (item.select = true), (item.pic = this.formValidate.image);
        });
      } else {
        data.forEach((item) => {
          item.select = true;
    ...
    // 立即生成
    generate(type, callback, newText, oldText) {
      generateAttrApi({ attrs: this.attrs,product_type: this.formValidate.product_type }, this.formValidate.id, type)
				.then((res) => {
					...
					} else if ((this.$route.params.id !== "0" && this.type != -1) || type == 1) {
    ...
    getPicD(pc) {
      ...
      this.images.map((item) => {
        if(item.file_type == 2) return this.$Message.warning("选择图片格式错误");
    ...
    handleDblclick(item, indexn, index) {
      this.editTagIndex = [index, indexn];
      this.editTagText = item;
      this.$nextTick(() => {
    ...
    // 编辑规格属性在回车或者失焦时调用立即生成
    handleEnter(newText, oldText) {
      // 生成规格
      this.attrs[this.editTagIndex[0]].detail[this.editTagIndex[1]] = newText;
      // 清空编辑状态
      this.editTagIndex = [];
      // 生成规格组合
      this.generate(1, (newText, oldText) => {
        // 遍历所有规格组合
        this.manyFormValidate.forEach((item, index) => {
          Object.keys(item).forEach((key) => {
            ...
            if (key === "detail" && typeof item[key] === "object") {
              Object.keys(item[key]).forEach((key2) => {
                if (typeof item[key][key2] === "string" && item[key][key2].includes(oldText)) {
                  // 替换
                  item[key][key2] = item[key][key2].replace(oldText, newText);
                }
              })
            }
          });
