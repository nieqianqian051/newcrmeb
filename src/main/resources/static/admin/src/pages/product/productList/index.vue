  ...
  <div class="article-manager">
              ...
              <div class="labelInput acea-row row-between-wrapper" @click="openGoodsLabel">
                <div style="width: 100%;">
                  <div v-if="goodsDataLabel.length">
                    <Tag closable v-for="(item,index) in goodsDataLabel" :key="index" @on-close="closeStoreLabel(item)">
                      {{ item.label_name }}
                    </Tag>
                  </div>
            ...
            <template>
              ...
              >
                ...
                <DropdownMenu slot="list">
                  ...
                  <DropdownItem name="7" v-if="artFrom.type === '6'" >删除商品 </DropdownItem>
                  <DropdownItem name="3" v-if="artFrom.type !== '6'">移到回收站</DropdownItem>
          ...
          <Form :model="batchData" :label-width="122">
            ...
            <FormItem v-if="menuActive === 2" label="商品标签：">
              <div class="select-tag" @click="openStoreLabel">
                <div v-if="storeDataLabel.length">
                  <Tag v-for="item in storeDataLabel" :key="item.id" closable @on-close="tagClose(item.id)">
                    {{ item.label_name }}
                  </Tag>
                </div>
            ...
            <FormItem v-if="menuActive === 7" label="自定义留言：">
              ...
              <div v-if="customBtn && batchData.system_form_id">
                <Table border :columns="formColumns" :data="formTypeList" ref="table" class="customTab" width="100%"
                       max-height="260">
    ...
    >
      <Form :model="auditForm" :label-width="80">
        <div>
          <Alert type="warning" show-icon>供应商商品首次审核时，售价、划线价默认为0，请检查后审核!</Alert>
        </div>
        <FormItem label="审核状态：">
      ...
      productGetVerifyApi(this.auditForm.id).then(res=>{
        if(res.data.verify == 1){
          this.$Modal.confirm({
            ...
            onOk: () => {
              productReviewApi(this.auditForm.id,this.auditForm).then(res=>{
                this.auditForm.id = '';
